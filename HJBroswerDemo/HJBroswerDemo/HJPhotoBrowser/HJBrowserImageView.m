...
@implementation HJBrowserImageView
{
    __weak HJWaitingView *_waitingView;
    BOOL _didCheckSize;
...
- (void)layoutSubviews
{
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        if (!_scroll) {
            ...
            scroll.backgroundColor = HJPhotoBrowserBackgrounColor;
            _scroll = scroll;
...
{
    HJWaitingView *waiting = [[HJWaitingView alloc] init];
    waiting.bounds = CGRectMake(0, 0, 100, 100);
    waiting.mode = HJWaitingViewProgressMode;
...
- (void)prepareForImageViewScaling
{
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = HJPhotoBrowserBackgrounColor;
